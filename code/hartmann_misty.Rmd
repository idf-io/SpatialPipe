...
source("custom_wrappers_and_functions.R")
```
...
```{r}
# Load AnnData & subset
h5ad <- readH5AD("../output-data/hartmann/hartmann.h5ad")
ds <- h5ad[, h5ad$point==example_point]
...
```{r}
misty.views %>% run_misty(results.folder=paste0(results.folder, "intra-para/point", as.character(example_point), "/"), seed=42)
```

# Visualize results

```{r}
misty.results <- collect_results(paste0(results.folder, "intra-para/point", as.character(example_point)))

saveRDS(misty.results, file = paste0(results.folder, "intra-para/point", as.character(example_point), "/hartmann_misty_intra-para_point", as.character(example_point), ".Rds"))

misty.results <- readRDS(paste0(results.folder, "intra-para/point", as.character(example_point), "/hartmann_misty_intra-para_point", as.character(example_point), ".Rds"))
...
for (point in unique(h5ad$point)) {
  ...
  create_initial_view(sample.xi) %>% add_paraview(sample.pos, l=radius, family="gaussian", zoi=0) %>% run_misty(paste0(results.folder, "intra-para/point/", point))

}

misty.results.all <- collect_results(list.dirs(results.folder, recursive=FALSE))
```

```{r}
saveRDS(misty.results.all, "output-data/hartmann/misty_intra-para/hartmann_misty_intra-para_all.Rds")
misty.results.all <- readRDS("output-data/hartmann/misty_intra-para/hartmann_misty_intra-para_all.Rds")
...
# Running MISTy via cell-type decomposition
...
create_initial_view(as.data.frame(oh)) %>% add_paraview(pos, l=radius, family="gaussian", zoi=0) %>% run_misty(results.folder = "output-data/hartmann/misty_cell-types_point8/", seed=42, bypass.intra = TRUE)
...
saveRDS(misty.results.ct.p8, "output-data/hartmann/misty_cell-types_point8/hartmann_misty_cell-types_point8.Rds")
